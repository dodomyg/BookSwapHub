...
router.post("/register", async (req, resp) => {
  ...
  try {
    ...
    resp
      .status(500)
      .json({ error: "Internal server error in User registration" });
  }
});
...
router.post("/login", async (req, resp) => {
  ...
  try {
    ...
    if (!alreadyUser) {
      return resp
        .status(404)
        .json({ message: "No user found with this username, register first" });
    }
    const comparePw = await bcrypt.compare(password, alreadyUser.password);
    if (!comparePw) {
      return resp.status(400).json({ message: "Incorrect Password" });
    }
    const token = await jwt.sign({ id: alreadyUser._id }, process.env.KEY, {
      expiresIn: "5h",
    });
...
router.post("/preferences", verifyToken, async (req, resp) => {
  const { preferences } = req.body;
  const userId = req.userId;
  try {
    const fetchUser = await USER.findByIdAndUpdate(
      { _id: userId },
    ...
    return resp.status(200).json({
      ...
      preferences: fetchUser.preferences,
    });
  } catch (error) {
    console.log(error);
    resp.status(500).json({ error: "Internal server error in preferences" });
  }
});
